...
public abstract class AbstractHashMap<K,V> extends AbstractMap<K,V> {
  ...
  protected double maxLoadFactor;		//user defined max load (0 < load < 1)
  
  /** Creates a hash table with the given capacity and prime factor. */
  public AbstractHashMap(int cap, int p) {
    prime = p;
    capacity = cap;
    maxLoadFactor = .5;		//default max load of 0.5	
  ...
  public void adjustMaxLoad(double load) 
  {
	    // user defined max load (0 < load < 1)
	    if (load <= 0 || load >= 1) {
	        System.out.println("New max. load must be > 0 and < 1.");
	        return;
	    } else {
	    	maxLoadFactor = load;
	    }
  ...
  public V put(K key, V value) {
    ...
    if (n > capacity * maxLoadFactor)	//if (n > capacity / 2)     // keep load factor <= 0.5
  ...
  private void resize(int newCap) {
	  System.out.printf("Resizing to %s\n",newCap);
